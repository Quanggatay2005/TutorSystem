...
import { Send, Search, X, MessageSquare, ArrowLeft, UserPlus } from 'lucide-react';
import { mockData } from '../data/mockData';
import { useFollow } from '../context/FollowContext'; // Bỏ comment nếu bạn đã có file này
...
const Message = () => {
  ...
  }, [searchInput, isFollowing]); // Thêm isFollowing vào dependency nếu dùng thật

  // Lấy data người đang chat
  const selectedTutor = mockData.tutors.find(t => t.id === selectedTutorId);
  const selectedConvo = selectedTutorId ? (conversations[selectedTutorId] || { messages: [] }) : { messages: [] };
  // Auto scroll
  useEffect(() => {
    if (chatEndRef.current) {
      chatEndRef.current.scrollIntoView({ behavior: 'smooth' });
    }
  }, [conversations, selectedTutorId]); // Scroll khi conversations thay đổi
  ...
  const handleSendMessage = (text, sender = 'user') => {
    ...
    setConversations(prev => {
      // 1. Lấy cuộc trò chuyện hiện tại của Tutor này (hoặc tạo mới nếu chưa có)
      const currentConvo = prev[selectedTutorId] || { messages: [] };
      
      // 2. Tạo bản sao mới hoàn toàn (Immutable Update)
      // KHÔNG DÙNG .push() ở đây
      return {
        ...prev, // Giữ nguyên các cuộc trò chuyện của người khác
        [selectedTutorId]: {
          ...currentConvo,
          messages: [
            ...currentConvo.messages, // Copy tin nhắn cũ
            { from: sender, text: text } // Thêm tin nhắn mới vào cuối
          ]
        }
      };
    });
  ...
  //   // 2. Giả lập Bot trả lời sau 1s
  //   setTimeout(() => {
  //     const replies = [
  //       'Chào bạn, mình có thể giúp gì?',
  //       'Mình nhận dạy kèm môn này, bạn rảnh buổi nào?',
  //       'Ok bạn, để mình xem lại lịch nhé.',
  //       'Cảm ơn bạn đã liên hệ! Học phí là 200k/buổi nhé.',
  //       'Bạn kết bạn Zalo với mình để trao đổi kỹ hơn nha.',
  //     ];
  //     const reply = replies[Math.floor(Math.random() * replies.length)];
      
  //     // Gọi lại hàm gửi tin nhắn với role là bot
  //     handleSendMessage(reply, 'bot');
  //   }, 1000);
  // };
  const SCRIPT_REPLIES = [
    'Chào bạn, mình có thể giúp gì cho việc học của bạn?',     // Câu 1
    'Môn này mình nhận dạy kèm vào chiều thứ 4 nhé.', // Câu 2
    'Học phí là 200k/buổi (90 phút) bạn nhé.',               // Câu 3
    'Bạn muốn học Online qua Google Meet hay Offline?',       // Câu 4
    'Ok chốt nhé! Bạn kết bạn Zalo mình để xếp lịch chi tiết nha.' // Câu 5
  ];
  ...
  const onSend = () => {
    ...
    // 2. Bot trả lời theo kịch bản tuần tự
    setTimeout(() => {
      if (!selectedTutorId) return;

      setConversations(prev => {
        // Lấy cuộc trò chuyện hiện tại để đếm số tin nhắn Bot đã gửi
        const currentConvo = prev[selectedTutorId] || { messages: [] };
        
        // Đếm số tin nhắn từ 'bot'
        const botMsgCount = currentConvo.messages.filter(m => m.from === 'bot').length;
        
        // Chọn câu trả lời dựa trên số lượng tin nhắn đã gửi
        // Dùng toán tử % (chia lấy dư) để nếu hết kịch bản thì quay lại câu đầu tiên
        // Hoặc dùng Math.min để dừng lại ở câu cuối
        let replyText = "";
        
        if (botMsgCount < SCRIPT_REPLIES.length) {
            replyText = SCRIPT_REPLIES[botMsgCount];
        } else {
            // Nếu hết kịch bản thì trả lời câu mặc định hoặc lặp lại câu cuối
            replyText = "Mình đã cung cấp đủ thông tin rồi, bạn cần gì nữa không?";
        }
  ...
  return (
    <div className="h-[calc(100vh-140px)] min-h-[500px] w-full bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex animate-fade-in">
      ...
      <div className={`w-full md:w-1/3 lg:w-1/4 flex-col border-r border-gray-100 bg-white ${selectedTutorId ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {displayedTutors.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400 p-6 text-center">
              <UserPlus size={40} className="mb-3 opacity-20"/>
              <p className="text-sm font-medium">Chưa có ai.</p>
            </div>
          ) : (
            displayedTutors.map(tutor => {
              const hasChat = conversations[tutor.id];
      ...
      <div className={`flex-1 flex-col bg-[#f8fafc] ${!selectedTutorId ? 'hidden md:flex' : 'flex'}`}>
        {selectedTutor ? (
          <>
            {/* Chat Header */}
            <div className="p-3 bg-white border-b border-gray-100 flex items-center justify-between shadow-sm z-10">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
               <div className="text-center py-6">
                 <img src={selectedTutor.avatar} className="w-20 h-20 rounded-full mx-auto mb-3 object-cover shadow-sm" alt=""/>
                 <p className="text-gray-500 text-xs">Bạn đang trò chuyện với <b>{selectedTutor.name}</b></p>
                 <p className="text-gray-400 text-[10px]">Chuyên môn: {selectedTutor.subjects ? selectedTutor.subjects.join(', ') : 'N/A'}</p>
               </div>
